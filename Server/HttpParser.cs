...
namespace HttpMessageParser
{
    public class HttpParser : IRequestParser
    {
        public HttpRequest ParseRequest(string request)
        {
            ...
            if (string.IsNullOrWhiteSpace(request)) {throw new ArgumentException("Request content cannot be null or empty", nameof(request));}
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                if (!isBody)
                {
                    ...
                    if (heaParts.Length == 2 && !string.IsNullOrWhiteSpace(heaParts[0]) && !string.IsNullOrWhiteSpace(heaParts[1]))
                    {
